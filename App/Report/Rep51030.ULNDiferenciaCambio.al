report 51030 "ULN Adjust Exchange Rates"
{
    // No. yyyy.mm.dd  Developer Company    DocNo.       Version     Description
    // -----------------------------------------------------------------------------------------------------
    // 001 2015 11 16  REG       ULN                     V.001       Add Localization Posting.
    // 002 2015 12 17  REG       ULN                     V.001       Adjmt. by Document Posting Group.
    // 003 2015 12 17  REG       ULN                     V.001       Add Referencial Documents.
    // -----------------------------------------------------------------------------------------------------
    ApplicationArea = Basic, Suite;
    Caption = 'Diferencia Cambio Perú';
    ...
    dataset
    {
        dataitem(Currency; Currency)
        {
            ...
            dataitem("Bank Account"; "Bank Account")
            {
                ...
                begin
                    ...
                    //001
                    gCurrFactor := 0;
                    if gBankPostingGroup.Get("Bank Account"."Bank Acc. Posting Group") then begin
                        case gBankPostingGroup."Currency Exch. Type" of
                            gBankPostingGroup."Currency Exch. Type"::Active:
                                gCurrFactor := 1 / gActiveADJTTC;
                            gBankPostingGroup."Currency Exch. Type"::Passive:
                                gCurrFactor := 1 / gPasiveADJTTC;
                        end;
                    end;
                    //001
                    //003
                    "#InitReferencialValues"('', "Bank Account"."No.", 3);
                    //003
                    ...
                    AdjAmount :=
                      Round(
                        CurrExchRate.ExchangeAmtFCYToLCYAdjmt(
                          PostingDate, Currency.Code, "Balance at Date", gCurrFactor)) -//001 Currency."Currency Factor")) -
                      "Balance at Date (LCY)";
                    ...
                    if AdjAmount <> 0 then begin
                        ...
                        //003
                        GenJnlLine."Ref. Document No." := gRefDocNo;
                        GenJnlLine."Ref. Source No." := gRefSourceNo;
                        GenJnlLine."Ref. Source Type" := gRefSourceType;
                        //003
            ...
            begin
                //001
                if gActiveADJTTC = 0 then Error('Falta colocar el tipo de cambio Activo');
                if gPasiveADJTTC = 0 then Error('Falta colocar el tipo de cambio Pasivo');
                if GetFilter(Code) = '' then Error('Falta filtrar por una moneda');
                //001
                CheckPostingDate;
        ...
        dataitem(Customer; Customer)
        {
            ...
            begin
                ...
                //LOF::20.04.18 ++
                CustLedgerEntry.SetFilter("Date Filter", '..' + Format(EndDate));
        ...
        dataitem(Vendor; Vendor)
        {
            ...
            begin
                ...
                //LOF::20.04.18 ++
                VendorLedgerEntry.SetFilter("Date Filter", '..' + Format(EndDate));
                //LOF::20.04.18 ++
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    group("Adjustment Period")
                    {
                        ...
                        field(EndingDate; EndDateReq)
                        {
                            ...
                            begin
                                ...
                                //MESSAGE(FORMAT(EndDateReq));
                                //MESSAGE('copia'+FORMAT(PostingDate));
                                //BEGIN ULN::CCL 001 ++
                                //gPasiveADJTTC:=0.0;
                                //gActiveADJTTC:=0.0;
                                gErrorDivisa := false;
                                recHistDivisas2.Reset;
                                recHistDivisas2.SetRange("Starting Date", PostingDate);
                                if recHistDivisas2.FindLast then begin
                                    //gPasiveADJTTC:=recHistDivisas."T.C Pasivo";
                                    //gActiveADJTTC:=recHistDivisas."T.C Activo";
                                end else begin
                                    gErrorDivisa := true;
                                    Error('No existe divisa para esta fecha', true);
                                end;
                                //END ULN::CCL 001 ++
                            end;
                        }
                    ...
                    field(gPasiveADJTTC; gPasiveADJTTC)
                    {
                        ApplicationArea = Basic, Suite;
                        Caption = 'T.C. Pasivo', Comment = 'ESM="T.C. Activo"';
                        DecimalPlaces = 3 : 3;
                    }
                    field(gActiveADJTTC; gActiveADJTTC)
                    {
                        ApplicationArea = All;
                        Caption = 'T.C. Activo', Comment = 'ESM="T.C. Activo"';
                        DecimalPlaces = 3 : 3;
                    }
                }
            }
    ...
    var
        ...
        ULN: Integer;
        gPasiveADJTTC: Decimal;
        gActiveADJTTC: Decimal;
        ULN001: Label 'Falta colocar el tipo de cambio Activo';
        ULN002: Label 'Falta colocar el tipo de cambio Pasivo';
        ULN003: Label 'Falta filtrar por una moneda';
        gCurrFactor: Decimal;
        gBankPostingGroup: Record "Bank Account Posting Group";
        gCustPostingGroup: Record "Customer Posting Group";
        ...
        gCuAnalitycsAutCalculation: Codeunit "Analitycs & Aut Calculation";
        lineno: Integer;
        gsuma: Decimal;
        grecDefaultDimension: Record "Default Dimension";
        recHistDivisas: Record "Hist. Divisas";
        gErrorDivisa: Boolean;
        recHistDivisas2: Record "Currency Exchange Rate";
    ...
    var
        lclVendLedgerEntry: Record "Vendor Ledger Entry";
        lclCustLedgEntry: Record "Cust. Ledger Entry";
    begin
        with GenJnlLine do
            if PostingAmount <> 0 then begin
                ...
                //003
                "Ref. Document No." := gRefDocNo;
                "Ref. Source No." := gRefSourceNo;
                "Ref. Source Type" := gRefSourceType;
                if "Ref. Source Type" = "Ref. Source Type"::Vendor then begin
                    lclVendLedgerEntry.Reset;
                    lclVendLedgerEntry.SetRange("Vendor No.", "Ref. Source No.");
                    lclVendLedgerEntry.SetRange("Document No.", "Ref. Document No.");
                    if lclVendLedgerEntry.FindSet then
                        "External Document No." := lclVendLedgerEntry."External Document No.";
                end else
                    if "Ref. Source Type" = "Ref. Source Type"::Customer then begin
    ...
    begin
        ...
        with ExchRateAdjReg do begin
            ...
            "Currency Factor" := gCurrFactor;//001 Currency2."Currency Factor";
    ...
    begin
        ...
        OK := AdjExchRateBuffer.Get(CurrencyCode2, PostingGroup2, DimEntryNo, Postingdate2, ICCode, gRefSourceNo, gRefDocNo);
        ...
        //003
        AdjExchRateBuffer."Ref. Document No." := gRefDocNo;
        AdjExchRateBuffer."Ref. Source No." := gRefSourceNo;
        AdjExchRateBuffer."Ref. Source Type" := gRefSourceType;
        //003
        ...
        if AdjExchRateBuffer.Find('-') then begin
            ...
            repeat
                ...
                OK :=
                  AdjExchRateBuffer2.Get(
                    ...
                    AdjExchRateBuffer."IC Partner Code", AdjExchRateBuffer."Ref. Source No.", AdjExchRateBuffer."Ref. Document No.");
                //004
            ...
            if AdjExchRateBuffer2.Find('-') then
                repeat
                    ...
                    //003
                    "#InitReferencialValues"(AdjExchRateBuffer2."Ref. Document No.", AdjExchRateBuffer2."Ref. Source No.", AdjExchRateBuffer2."Ref. Source Type");
                    //003
                        ...
                        case AdjustAccType of
                            1: // Customer
                                begin
                                    ...
                                    gsuma := gsuma + AdjExchRateBuffer2.AdjAmount;
                                end;
                            2: // Vendor
                                begin
    ...
    local procedure AdjustVATEntries(VATType: Enum "General Posting Type";
UseTax: Boolean)
    begin
        Clear(VATEntry2);
    ...
    begin
        ...
        if GenJnlLine."Account Type" = GenJnlLine."Account Type"::"Bank Account" then
            gCuAnalitycsAutCalculation."UpdateDimensionsbyaCode"(GenJnlLine, 8);
        gCuAnalitycsAutCalculation."UpdateDimensionsbyaCode"(GenJnlLine, 1);
        //004
    ...
    begin
        with CusLedgerEntry do begin
            ...
            //001
            gCurrFactor := 0;
            if gCustPostingGroup.Get(CustLedgerEntry."Customer Posting Group") then begin
                case gCustPostingGroup."Currency Exch. Type" of
                    gCustPostingGroup."Currency Exch. Type"::Active:
                        gCurrFactor := 1 / gActiveADJTTC;
                    gCustPostingGroup."Currency Exch. Type"::Passive:
                        gCurrFactor := 1 / gPasiveADJTTC;
                end;
            end;
            //001
            //003
            "#InitReferencialValues"(CustLedgerEntry."Document No.", CustLedgerEntry."Customer No.", 1);
            //003
...
END;*/
            //001
            if "Adjusted Currency Factor" <> gCurrFactor then begin
                "Adjusted Currency Factor" := gCurrFactor;
                Modify;
            end;
            //001
            ...
            AdjAmount :=
              Round(
                CurrExchRate.ExchangeAmtFCYToLCYAdjmt(
                  PostingDate2, Currency2.Code, "Remaining Amount", gCurrFactor)) -//001 Currency2."Currency Factor")) -
              "Remaining Amt. (LCY)";
                    ...
                    case true of
                        ...
                        (AdjAmount < 0):
                            if -AdjAmount <= OldAdjAmount then begin
                                ...
                                AdjExchRateBufIndex :=
                                  AdjExchRateBufferUpdate(
                                    "Currency Code", CusLedgerEntry."Customer Posting Group",//002  Customer."Customer Posting Group",
    ...
    begin
        with VendLedgerEntry do begin
            ...
            //001
            gCurrFactor := 0;
            if gVendorPostingGroup.Get(VendLedgerEntry."Vendor Posting Group") then begin
                case gVendorPostingGroup."Currency Exch. Type" of
                    gVendorPostingGroup."Currency Exch. Type"::Active:
                        gCurrFactor := 1 / gActiveADJTTC;
                    gVendorPostingGroup."Currency Exch. Type"::Passive:
                        gCurrFactor := 1 / gPasiveADJTTC;
                end;
            end;
            //001
            ...
            //003
            "#InitReferencialValues"(VendLedgerEntry."Document No.", VendLedgerEntry."Vendor No.", 2);
            //003
            Currency2.Get("Currency Code");
            GainsAmount := 0;
            LossesAmount := 0;
...
END;*/
                    ...
                    case true of
                        ...
                        (AdjAmount < 0):
                            if -AdjAmount <= OldAdjAmount then begin
                                ...
                                AdjExchRateBufIndex :=
                                  AdjExchRateBufferUpdate(
                                    "Currency Code", VendLedgerEntry."Vendor Posting Group",//002 Vendor."Vendor Posting Group",
    ...
    local procedure "#InitReferencialValues"(pDocumentNo: Code[20]; pSourceNo: Code[20]; pSourceType: Option " ",Customer,Vendor,"Bank Account","Fixed Asset")
    begin
        //003
        gRefSourceType := gRefSourceType::" ";
        gRefDocNo := '';
        gRefSourceNo := '';
        gRefSourceType := pSourceType;
        gRefDocNo := pDocumentNo;
        gRefSourceNo := pSourceNo;
        //003
    end;
