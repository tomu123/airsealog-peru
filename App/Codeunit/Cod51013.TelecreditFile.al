codeunit 51013 "Massive Banks Payments"
{
    ...
    var
        ...
        lclText003: Label 'There is no line of Type Account Bank in the Payment Journal';
        lclText004: Label 'Enter the Account Number for Bank % 1';
        lclText005: Label 'Bank Account Number% 1, must have 20 digits without dash';
        lclImportePagar: Decimal;
        lclCampoTemporal: Text;
        lclText006: Label 'The Length of the Identity Document must have a length equal to %1 for Vendor % 2';
        lclText007: Label 'The Type of Identity Document (Sunat Document) on Vendor''s Card % 1 must have a value and/or be different from 0';
        lclText008: Label 'Bank Account Number %1 from Vendor %2, must have %3 digits without dash';
        lclText009: Label 'Interbank Account Number %1 from Vendor %2, must have %3 digits without dash';
        lclText010: Label 'Applies-to Doc. No. must have a value in line %1';
        lclText011: Label 'It was not found Applies-to Doc. No. in the Entry Ledger Vendor on line %1';
        lclText012: Label 'El Num. Doc. a Liquidar en la linea %1, no tiene un Núm. Doc. Externo de Proveedor';
        lclDashPosition: Integer;
        lclText013: Label 'Select a type of process for the bank %1';
        lclFechaProceso: Date;
        lclTipoProceso: Text;
        lclText014: Label 'The process date must be greater than today''s date %1';
        lclText015: Label 'The maximum allowed process date can not be longer in 6 months of Today %1';
        lclTextDia: Text[2];
        lclTextMes: Text[2];
        lclTextAnio: Text[4];
        lclText016: Label 'Select a processing time for the bank %1';
    ...
    var
        lcLineArray: array[50] of Text[80];
        lcWindow: Dialog;
        ...
        lcText0014: Label 'ingrese nombre del proveedor de la linea %1 en la ficha de proveedores.';
    begin
        Clear(lcLineArray);
        Clear(lcFile);
        ...
        with GenJnlLine do begin
            ...
            if FindSet() then
                repeat
                    ...
                    lcLineArray[2] := "Account No.";
                    if StrLen(lcLineArray[2]) < 20 then
                        lcLineArray[2] := lcLineArray[2] + PadStr('', 20 - StrLen(lcLineArray[2]), ' ');
                    ...
                    if "Applies-to Entry No." <> 0 then begin
                        ...
                        if lcRecVendLedgrEntry.FindSet() then begin //Open 0001
                            ...
                            if lcRecVendLedgrEntry."External document No." <> '' then begin
                                lcLineArray[4] := CopyStr(Format(lcRecVendLedgrEntry."External document No."), 1, 20);
                                if StrLen(lcLineArray[4]) < 20 then
                                    lcLineArray[4] := lcLineArray[4] + PadStr('', 20 - StrLen(lcLineArray[4]), ' ');
                            end else
                            ...
                            if lcRecVendLedgrEntry."Due Date" <> 0D then begin
                                ...
                                lcLineArray[5] := lcYear + lcMonth + lcDay;
                            end;

                            if StrLen(lcLineArray[5]) < 8 then
                                lcLineArray[5] := lcLineArray[5] + PadStr('', 8 - StrLen(lcLineArray[5]), ' ');
                        ...
                        if "External document No." <> '' then begin
                            lcLineArray[4] := CopyStr(Format("External document No."), 1, 20);
                            if StrLen(lcLineArray[4]) < 20 then
                                lcLineArray[4] := lcLineArray[4] + PadStr('', 20 - StrLen(lcLineArray[4]), ' ');
                        end else
                            Error(ErrorNoExistVendorEntryForAppliedEntryNo, "Applies-to doc. No.", "Line No.");
                        ...
                        //Fecha Vencimiento documento
                        if "Due Date" <> 0D then
                            lcLineArray[5] := Format("Due Date", 0, '<Year4><Month,2><Day,2>')
                        else
                            lcLineArray[5] := '        ';
                        //** ANTICIPOS end ****
                    end;
                    ...
                    if lcLineArray[3] = 'C' then
                        lcOperationAmount := Amount + ABS("Retention Amount")
                    else
                        lcOperationAmount := Amount - ABS("Retention Amount");

                    lcLineArray[7] := DelChr(Format(lcOperationAmount, 0, '<Precision,2:2><Standard Format,0>'), '=', ',.');
                    if StrLen(lcLineArray[7]) < 15 then
                        lcLineArray[7] := PadStr('', 15 - StrLen(lcLineArray[7]), '0') + lcLineArray[7];
                    ...
                    //Tienda
                    if Not VendorBankAccount."Check Manager" then begin
                        if lcLineArray[9] = '09' then
                            lcLineArray[12] := CopyStr(DelChr(VendorBankAccount."Bank Account No.", '=', '-'), 1, 3)
                        else
                            lcLineArray[12] := '   ';
                    end else
                        lcLineArray[12] := '000';
                    ...
                    if Not VendorBankAccount."Check Manager" then begin
                        if VendorBankAccount."Bank Account Type" in [VendorBankAccount."Bank Account Type"::"Current Account",
                            ...
                            if (StrLen(VendorBankAccount."Bank Account No.") <> 13) then
                                ...
                                lcLineArray[13] := CopyStr(DelChr(VendorBankAccount."Bank Account No.", '=', '-'), 4, StrLen(VendorBankAccount."Bank Account No."));
                        end else
                            if (VendorBankAccount."Bank Account Type" = VendorBankAccount."Bank Account Type"::"Interbank Account") then begin
                                if (StrLen(VendorBankAccount."Bank Account CCI") <> 20) then
                                    ...
                                    lcLineArray[13] := DelChr(VendorBankAccount."Bank Account CCI", '=', '-');
                            end;
                    end else
                        lcLineArray[13] := '                    ';

                    if StrLen(lcLineArray[13]) < 20 then
                        lcLineArray[13] := lcLineArray[13] + PadStr('', 20 - StrLen(lcLineArray[13]), ' ');
                    ...
                    lcLineArray[16] := lclRecVendor."VAT Registration No.";
                    if (lclRecVendor."VAT Registration Type" = '1') and (StrLen(lcLineArray[16]) <> 8) then
                        Error(lcText0013);

                    if (lcLineArray[14] = 'P') and (StrLen(lcLineArray[16]) = 11) and (lclRecVendor."VAT Registration Type" = '6') then begin
                        lcLineArray[16] := CopyStr("Account No.", 3, 8);
                        lcLineArray[2] := lcLineArray[16];
                        if StrLen(lcLineArray[2]) < 20 then
                            lcLineArray[2] := lcLineArray[2] + PadStr('', 20 - StrLen(lcLineArray[2]), ' ');
                        lcLineArray[15] := '01';
                    end;

                    if StrLen(lcLineArray[16]) < 15 then
                        lcLineArray[16] := lcLineArray[16] + PadStr('', 15 - StrLen(lcLineArray[16]), ' ');

                    //Nombre del Beneficiario
                    if lclRecGenJournalLine."Check Name" <> '' then//if Description <> '' then
                        lcLineArray[17] := CopyStr(lclRecGenJournalLine."Check Name", 1, 60)
                    else
                        if lclRecVendor.Name <> '' then
                            lcLineArray[17] := CopyStr(lclRecVendor.Name + ' ' + lclRecVendor."Name 2", 1, 60)
                        else
                            Error(lcText0014, "Line No.");


                    if StrLen(lcLineArray[17]) < 60 then
                        lcLineArray[17] := lcLineArray[17] + PadStr('', 60 - StrLen(lcLineArray[17]), ' ');

                    //En Blanco
                    lcLineArray[18] := '00';
                    //Retención
                    lcLineArray[19] := '0000000000000';
                    //Espacios en Blanco
                    lcLineArray[20] := '';
                    lcTotalLineText := '';
                    For lcI := 1 TO 20 do begin
                        lcTotalLineText += lcLineArray[lcI];
                    end;
                    //ULN::PC 001  2020.06.01 v.001 begin 
    ...
    var
        ...
        lcTotalRecords: integer;
        lcRecVendLedgrEntry: Record "Vendor Ledger Entry";
        lcText0001: Label 'Número Factura Proveedor no existe en Mov. Proveedor para la factura a liquidar %1 en la linea %2.';
        lcOperationAmount: Decimal;
        Employee: Record Employee;
        //EmployeeBankAccount: Record "Employee Bank Account";
        BankAccount: Record "Bank Account";
            ...
            repeat
                ...
                if lclRecVendor.FindSet() then begin
                    case lclRecVendor."VAT Registration Type" of
                        '1':
                            begin
                                //1. Ruc del proveedor
                                lcLineArray[1] := Format(lclRecVendor."VAT Registration No.");
                    ...
                    if StrLen(lcLineArray[1]) < 11 then
                        lcLineArray[1] := lcLineArray[1] + PadStr('', 11 - StrLen(lcLineArray[1]), ' ');

                    //2. Nombre del Proveedor
                    lcLineArray[2] := lclRecVendor.Name;
                    if StrLen(lcLineArray[2]) < 60 then
                        lcLineArray[2] := lcLineArray[2] + PadStr('', 60 - StrLen(lcLineArray[2]), ' ');
                                ...
                                case lclRecVendorBankAccount."Bank Account Type" of
                                    ...
                                    lclRecVendorBankAccount."Bank Account Type"::"Interbank Account":
                                        begin
                                            ...
                                            lcLineArray[9] := DelChr(lclRecVendorBankAccount."Bank Account CCI", '=', '-');
                                        end;
                                end;
                            end else
                                lcLineArray[6] := '4';
                            if lcLineArray[6] in ['2', '3'] then
                    ...
                    //lcLineArray[9] := '01234567891234567890';

                    //10. Email proveedor
                    lcLineArray[8] := CopyStr(lclRecVendor."E-Mail", 1, 30);
                    if StrLen(lcLineArray[8]) < 30 then
                        lcLineArray[8] := lcLineArray[8] + PadStr('', 30 - StrLen(lcLineArray[8]), ' ');

                end else
                    Error('El Cód. Proveedor ingresado en la linea %1, no existe ne la ficha de Proveedores', lclRecGenJournalLine."Line No.");
                ...
                if lclRecGenJournalLine."Applies-to doc. No." <> '' then begin
                    ...
                    if lclRecVendorLedgerEntry.FindSet() then begin
                        ...
                        if lclRecVendorLedgerEntry."External document No." <> '' then begin
                            ...
                            if StrLen(lclNumdoc) < 14 then
                                lcLineArray[3] := lclNumdoc + PadStr('', 14 - StrLen(lclNumdoc), ' ');
                        end else
                            Error('El Num. doc. a Liquidar en la linea %1, no tiene un Núm. doc. Externo de Proveedor'
                               , lclRecGenJournalLine."Line No.");

                        //4. Fecha Emision Factura
                        if lclRecVendorLedgerEntry."document Date" <> 0D then
                            lcLineArray[4] := Format(lclRecVendorLedgerEntry."document Date", 0, '<Year4><Month,2><Day,2>')
                        else
                            Error('El Num. doc. a Liquidar en la linea %1, no tiene fecha emisión'
                               , lclRecGenJournalLine."Line No.");

                        //13. Fecha vencimiento Factura
                        if lclRecVendorLedgerEntry."Due Date" <> 0D then begin
                            lcLineArray[13] := Format(lclRecVendorLedgerEntry."Due Date", 0, '<Year4><Month,2><Day,2>')
                        end else
                            Error('Debe seleccionar fecha vencimiento para la linea %1.', lclRecGenJournalLine."Line No.");
                    ...
                    if lclRecGenJournalLine."External document No." <> '' then begin
                        if STRPOS(lclRecGenJournalLine."External document No.", '-') > 0 then begin
                            ...
                            if StrLen(lclRecGenJournalLine."External document No.") < 14 then
                                lcLineArray[3] := lclRecGenJournalLine."External document No." + PadStr('', 14 - StrLen(lclRecGenJournalLine."External document No."), ' ')
                            else
                                lcLineArray[3] := lclRecGenJournalLine."External document No.";
                        end;
                    end else
                        Error('El Num. doc. a Liquidar en la linea %1, no tiene un Núm. doc. Externo de Proveedor'
                           , lclRecGenJournalLine."Line No.");
                    ...
                    if lclRecGenJournalLine."document Date" <> 0D then
                        lcLineArray[4] := Format(lclRecGenJournalLine."document Date", 0, '<Year4><Month,2><Day,2>')
                    else
                        Error('El Num. doc. a Liquidar en la linea %1, no tiene fecha emisión'
                           , lclRecGenJournalLine."Line No.");
                    ...
                    if lclRecGenJournalLine."Due Date" <> 0D then
                        lcLineArray[13] := Format(lclRecGenJournalLine."Due Date", 0, '<Year4><Month,2><Day,2>')
                    else
                        Error('Debe ingresar fecha de vencimiento para la linea %1.', lclRecGenJournalLine."Line No.");
                    //++ end
                end;

                //5. Monto a Pagar o Descontar
                if lclRecGenJournalLine.Amount < 0 then
                    lcLineArray[5] := fnGetFormatAmt(false, lclRecGenJournalLine.Amount + ABS(lclRecGenJournalLine."Retention Amount"))
                else
                    lcLineArray[5] := fnGetFormatAmt(false, lclRecGenJournalLine.Amount - ABS(lclRecGenJournalLine."Retention Amount"));
                //     DelChr(Format(lclRecGenJournalLine.Amount-ABS(lclRecGenJournalLine."Retention Amount")),'=',',.-');
                if StrLen(lcLineArray[5]) < 11 then
                    lcLineArray[5] := PadStr('', 11 - StrLen(lcLineArray[5]), '0') + lcLineArray[5];

                if (lclRecGenJournalLine.Amount - lclRecGenJournalLine."Retention Amount") < 0 then
                    lcLineArray[5] := '-' + lcLineArray[5];

                //7. Codigo de oficina

                //8. Codigo de Cuenta
                if StrLen(lcBankAccountNo) > 11 then
                    lcLineArray[7] := CopyStr(lcBankAccountNo, StrLen(lcBankAccountNo) - 11, StrLen(lcBankAccountNo))
                else
                    lcLineArray[7] := lcBankAccountNo + PadStr('', (11 - StrLen(lcBankAccountNo)), ' ');

                //9. Flag Simple Pay
                //lcLineArray[9] := '*';

                //11. Codigo de cuenta interbancaria
                lcLineArray[9] := lcLineArray[9] + PadStr('', (20 - StrLen(lcLineArray[9])), ' ');

                //12. Marca Factoring Electronico
                lcLineArray[12] := PadStr('', (30 - StrLen(lcLineArray[8])), ' ');
                lcTotalLineText := '';
                For lcI := 1 TO 20 do
                    lcTotalLineText += lcLineArray[lcI];
                //ULN::PC 001  2020.06.01 v.001 begin 
    ...
    var
        ...
        VendorWihtoutBank: Label 'El proveedor %1 de de linea %2 no tiene un banco creado en ficha de Banco Proveedores';
        ...
        ErrorNoExistVendorEntryForAppliedEntryNo: Label 'Número Factura Proveedor no existe en Mov. Proveedor para la factura a liquidar %1 en la linea %2.';
